...
***************************************/
    ...
    *  After initializing the SCB in EZI2C mode using this function, 
...
*  - SCB_EZI2C_STATUS_READ1 - Read transfer complete. The transfer 
...
*  - SCB_EZI2C_STATUS_WRITE1 - Write transfer complete. The buffer 
...
*  - SCB_EZI2C_STATUS_READ2 - Read transfer complete. The transfer
*    used the secondary slave address. The error condition status bit must be 
*    checked to ensure that read transfer was completed successfully.
*  - SCB_EZI2C_STATUS_WRITE2 - Write transfer complete. The buffer
*    content was modified. The transfer used the secondary slave address. 
*    The error condition status bit must be checked to ensure that write 
*    transfer was completed successfully.
*  - SCB_EZI2C_STATUS_BUSY - A transfer intended for the primary 
...
*  - SCB_EZI2C_STATUS_ERR - An error occurred during a transfer 
...
*    SCB_EZI2C_STATUS_ERR and SCB_EZI2C_STATUS_WRITE1/2 
*    is set. It is recommended to discard buffer content in this case.
*
* \globalvars
*  SCB_curStatus - used to store the current status of the EZI2C 
*  slave.
*
*******************************************************************************/
...
*
* \globalvars
*  SCB_addrBuf1 - used to store the primary 7-bit slave address 
*  value.
*
*******************************************************************************/
void SCB_EzI2CSetAddress1(uint32 address)
{
    #if(SCB_SECONDARY_ADDRESS_ENABLE)
    {
        ...
        SCB_RX_MATCH_REG = 
                        SCB_EzI2CUpdateRxMatchReg(address, (uint32) SCB_addrBuf2);
    }
    #else
    {
        uint32 matchReg;
        ...
        /* Set address. */
        matchReg &= (uint32) ~SCB_RX_MATCH_ADDR_MASK;
        matchReg |= (uint32)  SCB_GET_I2C_8BIT_ADDRESS(address);
...
*
* \globalvars
*  SCB_dataBuffer1 – the pointer to the buffer to be exposed to the
*  master on a primary address.
*  SCB_bufSizeBuf1 - the size of the buffer to be exposed to the 
*  master on a primary address.
*  SCB_protectBuf1 - the start index of the read-only region in the
...
void SCB_EzI2CSetReadBoundaryBuffer1(uint32 rwBoundary)
{
    ...
    *
    * \globalvars
    *  SCB_addrBuf2 - used to store the secondary 7-bit slave address 
    *  value.
    *
    *******************************************************************************/
    void SCB_EzI2CSetAddress2(uint32 address)
    {
        ...
        SCB_RX_MATCH_REG = 
                        SCB_EzI2CUpdateRxMatchReg((uint32) SCB_addrBuf1, address);
    }
    ...
    *
    * \globalvars
    *  SCB_dataBuffer2 – the pointer to the buffer to be exposed to the
    *  master on a secondary address.
    *  SCB_bufSizeBuf2 - the size of the buffer to be exposed to the 
    *  master on a secondary address.
    *  SCB_protectBuf2 - the start index of the read-only region in the

...
static void SPI_1_ScbModeStop(void)
{
    ...
    *******************************************************************************/
    void SPI_1_SetPins(uint32 mode, uint32 subMode, uint32 uartEnableMask)
    {
        ...
        {
            SPI_1_RX_SDA_MOSI_HSIOM_SEL_GPIO,
            SPI_1_TX_SCL_MISO_HSIOM_SEL_GPIO,
            0u,
            0u,
            0u,
            0u,
            ...
            (SPI_1_SCB_MODE_EZI2C == mode))
        {
            ...
            hsiomSel[SPI_1_RX_SDA_MOSI_PIN_INDEX] = SPI_1_RX_SDA_MOSI_HSIOM_SEL_I2C;
            pinsDm  [SPI_1_RX_SDA_MOSI_PIN_INDEX] = SPI_1_PIN_DM_OD_LO;
        #elif (SPI_1_RX_WAKE_SDA_MOSI_PIN)
            hsiomSel[SPI_1_RX_WAKE_SDA_MOSI_PIN_INDEX] = SPI_1_RX_WAKE_SDA_MOSI_HSIOM_SEL_I2C;
            pinsDm  [SPI_1_RX_WAKE_SDA_MOSI_PIN_INDEX] = SPI_1_PIN_DM_OD_LO;
        #else
        #endif /* (SPI_1_RX_SDA_MOSI_PIN) */
        
        #if (SPI_1_TX_SCL_MISO_PIN)
            hsiomSel[SPI_1_TX_SCL_MISO_PIN_INDEX] = SPI_1_TX_SCL_MISO_HSIOM_SEL_I2C;
            pinsDm  [SPI_1_TX_SCL_MISO_PIN_INDEX] = SPI_1_PIN_DM_OD_LO;
        ...
        {
        #if (SPI_1_RX_SDA_MOSI_PIN)
            hsiomSel[SPI_1_RX_SDA_MOSI_PIN_INDEX] = SPI_1_RX_SDA_MOSI_HSIOM_SEL_SPI;
        #elif (SPI_1_RX_WAKE_SDA_MOSI_PIN)
            hsiomSel[SPI_1_RX_WAKE_SDA_MOSI_PIN_INDEX] = SPI_1_RX_WAKE_SDA_MOSI_HSIOM_SEL_SPI;
        #else
        #endif /* (SPI_1_RX_SDA_MOSI_PIN) */
        
        #if (SPI_1_TX_SCL_MISO_PIN)
            hsiomSel[SPI_1_TX_SCL_MISO_PIN_INDEX] = SPI_1_TX_SCL_MISO_HSIOM_SEL_SPI;
        #endif /* (SPI_1_TX_SCL_MISO_PIN) */
            ...
            {
                /* Slave */
                pinsDm[SPI_1_RX_SDA_MOSI_PIN_INDEX] = SPI_1_PIN_DM_DIG_HIZ;
                pinsDm[SPI_1_TX_SCL_MISO_PIN_INDEX] = SPI_1_PIN_DM_STRONG;
                ...
                /* Disable input buffer */
                 pinsInBuf |= SPI_1_TX_SCL_MISO_PIN_MASK;
            #endif /* (SPI_1_TX_SCL_MISO_PIN) */
            }
            else 
            {
                /* (Master) */
                pinsDm[SPI_1_RX_SDA_MOSI_PIN_INDEX] = SPI_1_PIN_DM_STRONG;
                pinsDm[SPI_1_TX_SCL_MISO_PIN_INDEX] = SPI_1_PIN_DM_DIG_HIZ;
            ...
            #if (SPI_1_RX_SDA_MOSI_PIN)
                pinsInBuf |= SPI_1_RX_SDA_MOSI_PIN_MASK;
            #elif (SPI_1_RX_WAKE_SDA_MOSI_PIN)
                pinsInBuf |= SPI_1_RX_WAKE_SDA_MOSI_PIN_MASK;
            #else
            #endif /* (SPI_1_RX_SDA_MOSI_PIN) */
        ...
        else /* UART */
        {
            if (SPI_1_UART_MODE_SMARTCARD == subMode)
            {
                /* SmartCard */
            #if (SPI_1_TX_SCL_MISO_PIN)
                hsiomSel[SPI_1_TX_SCL_MISO_PIN_INDEX] = SPI_1_TX_SCL_MISO_HSIOM_SEL_UART;
                pinsDm  [SPI_1_TX_SCL_MISO_PIN_INDEX] = SPI_1_PIN_DM_OD_LO;
            #endif /* (SPI_1_TX_SCL_MISO_PIN) */
            }
            else /* Standard or IrDA */
            {
                ...
                {
                #if (SPI_1_RX_SDA_MOSI_PIN)
                    hsiomSel[SPI_1_RX_SDA_MOSI_PIN_INDEX] = SPI_1_RX_SDA_MOSI_HSIOM_SEL_UART;
                    pinsDm  [SPI_1_RX_SDA_MOSI_PIN_INDEX] = SPI_1_PIN_DM_DIG_HIZ;
                #elif (SPI_1_RX_WAKE_SDA_MOSI_PIN)
                    hsiomSel[SPI_1_RX_WAKE_SDA_MOSI_PIN_INDEX] = SPI_1_RX_WAKE_SDA_MOSI_HSIOM_SEL_UART;
                    pinsDm  [SPI_1_RX_WAKE_SDA_MOSI_PIN_INDEX] = SPI_1_PIN_DM_DIG_HIZ;
                #else
                #endif /* (SPI_1_RX_SDA_MOSI_PIN) */
                }
    ...
    /* Note: the DR register settings do not effect the pin output if HSIOM is other than GPIO */
        ...
        SPI_1_SET_HSIOM_SEL(SPI_1_RX_SDA_MOSI_HSIOM_REG,
                                       ...
                                       SPI_1_RX_SDA_MOSI_HSIOM_POS,
                                        hsiomSel[SPI_1_RX_SDA_MOSI_PIN_INDEX]);

        SPI_1_uart_rx_i2c_sda_spi_mosi_SetDriveMode((uint8) pinsDm[SPI_1_RX_SDA_MOSI_PIN_INDEX]);

        #if (!SPI_1_CY_SCBIP_V1)
            SPI_1_SET_INP_DIS(SPI_1_uart_rx_i2c_sda_spi_mosi_INP_DIS,
                                         SPI_1_uart_rx_i2c_sda_spi_mosi_MASK,
                                         (0u != (pinsInBuf & SPI_1_RX_SDA_MOSI_PIN_MASK)));
        #endif /* (!SPI_1_CY_SCBIP_V1) */
        ...
        SPI_1_SET_HSIOM_SEL(SPI_1_RX_WAKE_SDA_MOSI_HSIOM_REG,
                                       ...
                                       hsiomSel[SPI_1_RX_WAKE_SDA_MOSI_PIN_INDEX]);

        SPI_1_uart_rx_wake_i2c_sda_spi_mosi_SetDriveMode((uint8)
                                                               pinsDm[SPI_1_RX_WAKE_SDA_MOSI_PIN_INDEX]);

        SPI_1_SET_INP_DIS(SPI_1_uart_rx_wake_i2c_sda_spi_mosi_INP_DIS,
                                     SPI_1_uart_rx_wake_i2c_sda_spi_mosi_MASK,
                                     (0u != (pinsInBuf & SPI_1_RX_WAKE_SDA_MOSI_PIN_MASK)));

         /* Set interrupt on falling edge */
        SPI_1_SET_INCFG_TYPE(SPI_1_RX_WAKE_SDA_MOSI_INTCFG_REG,
                                        SPI_1_RX_WAKE_SDA_MOSI_INTCFG_TYPE_MASK,
    ...
    #if (SPI_1_TX_SCL_MISO_PIN)
        SPI_1_SET_HSIOM_SEL(SPI_1_TX_SCL_MISO_HSIOM_REG,
                                       SPI_1_TX_SCL_MISO_HSIOM_MASK,
                                       SPI_1_TX_SCL_MISO_HSIOM_POS,
                                        hsiomSel[SPI_1_TX_SCL_MISO_PIN_INDEX]);

        SPI_1_uart_tx_i2c_scl_spi_miso_SetDriveMode((uint8) pinsDm[SPI_1_TX_SCL_MISO_PIN_INDEX]);

    #if (!SPI_1_CY_SCBIP_V1)
        SPI_1_SET_INP_DIS(SPI_1_uart_tx_i2c_scl_spi_miso_INP_DIS,
                                     SPI_1_uart_tx_i2c_scl_spi_miso_MASK,
                                    (0u != (pinsInBuf & SPI_1_TX_SCL_MISO_PIN_MASK)));
    #endif /* (!SPI_1_CY_SCBIP_V1) */
    #endif /* (SPI_1_RX_SDA_MOSI_PIN) */

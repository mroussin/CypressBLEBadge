...
#include "cyapicallbacks.h"
    ...
    /*******************************************************************************
    * Function Name: SCB_EZI2C_STRETCH_ISR
    ****************************************************************************//**
    *
    ...
    *******************************************************************************/
    CY_ISR_PROTO(SCB_EZI2C_STRETCH_ISR)
    {
    ...
    #ifdef SCB_EZI2C_STRETCH_ISR_ENTRY_CALLBACK
        SCB_EZI2C_STRETCH_ISR_EntryCallback();
    #endif /* SCB_EZI2C_STRETCH_ISR_ENTRY_CALLBACK */

    #if(SCB_SECONDARY_ADDRESS_ENABLE_CONST)
        /* Variable intended to be used with either buffer */
        static volatile uint8 * SCB_dataBuffer; /* Pointer to data buffer              */
        static uint16 SCB_bufSizeBuf;           /* Size of buffer in bytes             */
        static uint16 SCB_protectBuf;           /* Start index of write protected area */

        static uint8 activeAddress;
        uint32 ackResponse;

        ackResponse = SCB_EZI2C_ACK_RECEIVED_ADDRESS;
    ...
    #else
        CY_SCB_CUSTOM_INTR_HANDLER();
    #endif /* !defined (CY_REMOVE_SCB_CUSTOM_INTR_HANDLER) */

        /* Make local copy of global variable */
        locIndex = SCB_EZI2C_GET_INDEX(activeAddress);

        /* Get interrupt sources */
        locIntrSlave = SCB_GetSlaveInterruptSource();
        locIntrCause = SCB_GetInterruptCause();
        ...
        */
        if(0u != (locIntrSlave & (SCB_INTR_SLAVE_I2C_ARB_LOST |
                                  SCB_INTR_SLAVE_I2C_BUS_ERROR)))
        {
            SCB_CTRL_REG &= (uint32) ~SCB_CTRL_ENABLED; /* Disable SCB block */

        #if(SCB_CY_SCBIP_V0)
            if(0u != ((uint8) SCB_EZI2C_STATUS_BUSY & SCB_curStatus))
        #endif /* (SCB_CY_SCBIP_V0) */
            {
                SCB_curStatus &= (uint8) ~SCB_EZI2C_STATUS_BUSY;
                SCB_curStatus |= (uint8)  SCB_EZI2C_STATUS_ERR;

                /* INTR_TX_EMPTY is enabled in the address phase to receive data */
                if(0u == (SCB_GetTxInterruptMode() & SCB_INTR_TX_EMPTY))
                {
                    /* Write complete */
                    if(SCB_indexBuf1 != SCB_offsetBuf1)
                    {
                        SCB_curStatus |= (uint8) SCB_INTR_SLAVE_I2C_WRITE_STOP;
                    }
                }
                else
                {
                    /* Read complete */
                    SCB_curStatus |= (uint8) SCB_INTR_SLAVE_I2C_NACK;
                }
            }
            ...
            SCB_CTRL_REG |= (uint32) SCB_CTRL_ENABLED;  /* Enable SCB block */
        }
        else
        {
            /* INTR_I2C_EC_WAKE_UP */
            /* Wakes up device from deep sleep */
            if(0u != (locIntrCause & SCB_INTR_CAUSE_I2C_EC))
            {
            ...
            if(0u != (locIntrCause & (SCB_INTR_CAUSE_RX | SCB_INTR_CAUSE_SLAVE)))
            {
                /* INTR_RX.NOT_EMPTY */
                /* Receives data byte-by-byte. Does not use RX FIFO capabilities */
                if (0u != (SCB_GetRxInterruptSourceMasked() & SCB_INTR_RX_NOT_EMPTY))
                {
                #if(SCB_SECONDARY_ADDRESS_ENABLE_CONST)
                    ...
                    */
                    if (0u == (locIntrCause & SCB_INTR_CAUSE_SLAVE))
                #endif /* (SCB_SECONDARY_ADDRESS_ENABLE_CONST) */
                    {
                        locByte = SCB_RX_FIFO_RD_REG;

                        switch(SCB_fsmState)
                        {
                        case SCB_EZI2C_FSM_BYTE_WRITE:
                            if(0u != locBufSize)
                            {
                                /* Store data byte and ACK */
                                SCB_I2C_SLAVE_GENERATE_ACK;

                                SCB_dataBuffer1[locIndex] = (uint8) locByte;
                                locIndex++;
                                locBufSize--;
                            }
                            else
                    ...
                    #if(SCB_SUB_ADDRESS_SIZE16_CONST)
                        case SCB_EZI2C_FSM_OFFSET_HI8:
                            ...
                            SCB_fsmState = SCB_EZI2C_FSM_OFFSET_LO8;

                            break;
                    #endif /* (SCB_SUB_ADDRESS_SIZE16_CONST) */

                        case SCB_EZI2C_FSM_OFFSET_LO8:

                            #if (SCB_SUB_ADDRESS_SIZE16)
                            {
                            ...
                            if(locByte < (uint32) SCB_bufSizeBuf1)
                            {
                                SCB_I2C_SLAVE_GENERATE_ACK;
                                ...
                                locBufSize = (uint16) ((locByte < SCB_protectBuf1) ?
                                                       (SCB_protectBuf1 - locByte) : (0u));

                            #if(SCB_CY_SCBIP_V0)

                                if(locBufSize < SCB_EZI2C_FIFO_SIZE)
                                {
                                    /* Set FSM state to receive byte by byte */
                                    SCB_fsmState = SCB_EZI2C_FSM_BYTE_WRITE;
                                }
                                /* Receive RX FIFO chunks */
                                else if(locBufSize == SCB_EZI2C_FIFO_SIZE)
                                {
                                    SCB_ENABLE_SLAVE_AUTO_DATA; /* NACK when RX FIFO is full */
                                    SCB_SetRxInterruptMode(SCB_NO_INTR_SOURCES);
                                }
                                else
                                {
                                    SCB_ENABLE_SLAVE_AUTO_DATA_ACK; /* Stretch when RX FIFO is full */
                                ...
                                SCB_EZI2C_SET_OFFSET(activeAddress, locIndex);
                            }
                            else
                            {
                ...
                else if (0u != (SCB_I2C_CTRL_REG & SCB_I2C_CTRL_S_READY_DATA_ACK))
                {
                    ...
                    */
                    if(0u != (locIntrCause & SCB_INTR_CAUSE_SLAVE))
                    {
                        ...
                        fifoIndex = SCB_GET_RX_FIFO_ENTRIES;
                        ...
                        #if(SCB_SECONDARY_ADDRESS_ENABLE)
                        {
                            ...
                            if(SCB_CHECK_INTR_SLAVE_MASKED(SCB_INTR_SLAVE_I2C_ADDR_MATCH))
                            {
                                ...
                                */
                                fifoIndex -= ((0u != SCB_GET_RX_FIFO_SR_VALID) ? (0u) : (1u));
                            }
                        }
                        #endif

                        SCB_DISABLE_SLAVE_AUTO_DATA;
                        endTransfer = SCB_EZI2C_CONTINUE_TRANSFER;
                    }
                    else
                    /* INTR_RX_FULL */
                    /* Continue transfer or disable INTR_RX_FULL to catch completion event. */
                    {
                        ...
                        locBufSize -= (uint16) SCB_EZI2C_FIFO_SIZE;

                        if(locBufSize <= SCB_EZI2C_FIFO_SIZE)
                        {
                            /* Send NACK when RX FIFO overflow */
                            fifoIndex   = locBufSize;
                            endTransfer = SCB_EZI2C_COMPLETE_TRANSFER;
                        }
                        else
                        {
                            /* Continue  transaction */
                            fifoIndex   = SCB_EZI2C_FIFO_SIZE;
                            endTransfer = SCB_EZI2C_CONTINUE_TRANSFER;
                        }
                    }
                    ...
                    for(; (0u != fifoIndex); fifoIndex--)
                    {
                        ...
                        SCB_dataBuffer1[locIndex] = (uint8) SCB_RX_FIFO_RD_REG;
                        locIndex++;
                    }

                    /* Complete transfer sending NACK when RX FIFO overflow */
                    if(SCB_EZI2C_COMPLETE_TRANSFER == endTransfer)
                    {
            ...
            if(0u != (locIntrCause & SCB_INTR_CAUSE_SLAVE))
            {
            ...
            #if(!SCB_SECONDARY_ADDRESS_ENABLE_CONST)
                if(0u != (locIntrSlave & SCB_INTR_SLAVE_I2C_STOP))
            #else
                /* Prevent triggering when matched address was NACKed */
                if((0u != (locIntrSlave & SCB_INTR_SLAVE_I2C_STOP)) &&
                   (0u != ((uint8) SCB_EZI2C_STATUS_BUSY & SCB_curStatus)))
            #endif
                {
                    /* Disable TX and RX interrupt sources */
                    SCB_SetRxInterruptMode(SCB_NO_INTR_SOURCES);
                    ...
                    locStatus = SCB_INTR_SLAVE_I2C_NACK;

                    /* Check if buffer content was modified: the address phase resets the locIndex */
                    if(locIndex != SCB_EZI2C_GET_OFFSET(activeAddress))
                    {
                        locStatus |= SCB_INTR_SLAVE_I2C_WRITE_STOP;
                    }

                    /* Complete read or write transaction */
                    locStatus &= locIntrSlave;
                    SCB_EZI2C_UPDATE_LOC_STATUS(activeAddress, locStatus);
                    locStatus |= (uint32)  SCB_curStatus;
                    locStatus &= (uint32) ~SCB_EZI2C_STATUS_BUSY;
                    SCB_curStatus = (uint8) locStatus;
                    ...
                    #if(SCB_SECONDARY_ADDRESS_ENABLE)
                    {
                        ...
                        SCB_EZI2C_SET_INDEX(activeAddress, locIndex);
                    }
                    #endif
                }
                ...
                */
                if(0u != (locIntrSlave & SCB_INTR_SLAVE_I2C_ADDR_MATCH))
                {
                    #if(SCB_SECONDARY_ADDRESS_ENABLE)
                    {
                        /* Read address byte from RX FIFO */
                        locByte = SCB_GET_I2C_7BIT_ADDRESS(SCB_RX_FIFO_RD_REG);

                        SCB_ClearRxInterruptSource(SCB_INTR_RX_NOT_EMPTY);

                        /* Check received address against device addresses */
                        if(SCB_addrBuf1 == locByte)
                        {
                            /* Set buffer exposed to primary slave address */
                            SCB_dataBuffer = SCB_dataBuffer1;
                            SCB_bufSizeBuf = SCB_bufSizeBuf1;
                            SCB_protectBuf = SCB_protectBuf1;

                            activeAddress = SCB_EZI2C_ACTIVE_ADDRESS1;
                        }
                        else if(SCB_addrBuf2 == locByte)
                        {
                            /* Set buffer exposed to secondary slave address */
                            SCB_dataBuffer = SCB_dataBuffer2;
                            SCB_bufSizeBuf = SCB_bufSizeBuf2;
                            SCB_protectBuf = SCB_protectBuf2;

                            activeAddress = SCB_EZI2C_ACTIVE_ADDRESS2;
                        }
                        else
                        {
                            /* Address does not match */
                            ackResponse = SCB_EZI2C_NACK_RECEIVED_ADDRESS;
                        }
                    }
                    #endif

                #if(SCB_SECONDARY_ADDRESS_ENABLE_CONST)
                    if(SCB_EZI2C_NACK_RECEIVED_ADDRESS == ackResponse)
                    {
                        ...
                        SCB_DISABLE_INTR_SLAVE(SCB_INTR_SLAVE_I2C_STOP);
                    #endif /* (!SCB_CY_SCBIP_V0) */
                ...
                #endif /* (SCB_SECONDARY_ADDRESS_ENABLE_CONST) */
                    {
                        ...
                        SCB_ENABLE_INTR_SLAVE(SCB_INTR_SLAVE_I2C_STOP);
                    #endif /* (!SCB_CY_SCBIP_V0) */
                ...
                SCB_ClearSlaveInterruptSource(locIntrSlave);
            }
            ...
            */
            if(0u != (SCB_GetInterruptCause() & SCB_INTR_CAUSE_TX))
            {
                ...
                do
                {
                    ...
                    if(0u != locBufSize)
                    {
                        SCB_TX_FIFO_WR_REG = (uint32) SCB_dataBuffer1[locIndex];
                        locIndex++;
                        locBufSize--;
                    }
                    else
                    {
                        SCB_TX_FIFO_WR_REG = SCB_EZI2C_OVFL_RETURN;
                    }
                }
                while(SCB_EZI2C_FIFO_SIZE != SCB_GET_TX_FIFO_ENTRIES);
    ...
    #ifdef SCB_EZI2C_STRETCH_ISR_EXIT_CALLBACK
        SCB_EZI2C_STRETCH_ISR_ExitCallback();
    #endif /* SCB_EZI2C_STRETCH_ISR_EXIT_CALLBACK */
    ...
    /*******************************************************************************
    * Function Name: SCB_EZI2C_NO_STRETCH_ISR
    ****************************************************************************//**
    *
    *  Handles the Interrupt Service Routine for the SCB EZI2C mode.
    ...
    *******************************************************************************/
    ...
    #ifdef SCB_EZI2C_NO_STRETCH_ISR_ENTRY_CALLBACK
        SCB_EZI2C_NO_STRETCH_ISR_EntryCallback();
    #endif /* SCB_EZI2C_NO_STRETCH_ISR_ENTRY_CALLBACK */
        ...
        */
                                  ...
                                  SCB_INTR_SLAVE_I2C_BUS_ERROR)))
        {
        ...
        #endif /* (SCB_CY_SCBIP_V0) */
            {
                ...
                if(0u != (SCB_EZI2C_FSM_WRITE_MASK & SCB_fsmState))
                {
                    /* Write complete */
                    if(SCB_indexBuf1 != SCB_offsetBuf1)
                    {
        ...
        else
        {
            ...
            */
            if(0u != (locIntrCause & SCB_INTR_CAUSE_RX))
            {
                ...
                case SCB_EZI2C_FSM_BYTE_WRITE:
                    ...
                    if(SCB_indexBuf1 < SCB_protectBuf1)
                    {
                        /* Stores received byte into buffer */
                        SCB_dataBuffer1[SCB_indexBuf1] = (uint8) locByte;
                        SCB_indexBuf1++;
                    }
                    else
                    {
                        /* Discard current byte and sets FSM state to discard following bytes */
                        SCB_fsmState = SCB_EZI2C_FSM_WAIT_STOP;
                    }
                ...
                case SCB_EZI2C_FSM_OFFSET_LO8:
                    ...
                    if(locByte < (uint32) SCB_bufSizeBuf1)
                    {
                        ...
                        SCB_offsetBuf1 = (uint16) locByte;
                        SCB_indexBuf1  = (uint16) locByte;
                    ...
                    else
                    {
                        ...
                        SCB_indexBuf1 = (uint16) SCB_offsetBuf1;
            ...
            if(0u != (locIntrSlave & SCB_INTR_SLAVE_I2C_START))
            {
            #if(!SCB_CY_SCBIP_V0)
                ...
                #else
                {
                    ...
                    SCB_ClearSlaveInterruptSource(SCB_INTR_SLAVE_I2C_ADDR_MATCH);
                }
                #endif
            #else
                /* Manage INTR_SLAVE.I2C_ADDR_MATCH as slave busy status */
                ...
                SCB_FAST_CLEAR_TX_FIFO;
                SCB_TX_FIFO_WR_REG = (uint32) SCB_dataBuffer1[SCB_offsetBuf1];

                /* Store buffer index to be handled by INTR_SLAVE.I2C_STOP */
                locByte = (uint32) SCB_indexBuf1;

                /* Update index: one byte is already in the TX FIFO */
                SCB_indexBuf1 = (uint16) SCB_offsetBuf1 + 1u;

                /* Enable INTR_TX.NOT_FULL to load TX FIFO */
                SCB_SetTxInterruptMode(SCB_INTR_TX_TRIGGER);

                /* Clear locIntrSlave after INTR.TX_TRIGGER is enabled */
                SCB_ClearSlaveInterruptSource(locIntrSlave);

                locIntrCause |= SCB_INTR_CAUSE_TX;
            }


            /* INTR_TX.TRIGGER */
            /* Transmits data to the master: loads data into the TX FIFO. The TX FIFO is loaded with data
            *  until used entries are less than SCB_TX_LOAD_SIZE. If index reaches end of the
            *  buffer the 0xFF is sent to the end of transfer.
            */
            if(0u != (locIntrCause & SCB_INTR_CAUSE_TX))
            {
                /* Put data into TX FIFO until there is room */
                do
                {
                    /* Check transmit buffer range */
                    if(SCB_indexBuf1 < SCB_bufSizeBuf1)
                    {
                        SCB_TX_FIFO_WR_REG = (uint32) SCB_dataBuffer1[SCB_indexBuf1];
                        SCB_indexBuf1++;
                    }
                    else
                    {
                ...
                while(SCB_TX_LOAD_SIZE != SCB_GET_TX_FIFO_ENTRIES);

                SCB_ClearTxInterruptSource(SCB_INTR_TX_TRIGGER);
            }
            ...
            if(0u != (locIntrSlave & SCB_INTR_SLAVE_I2C_STOP))
            {
                ...
                if((locByte != SCB_offsetBuf1) &&
                   (0u != (SCB_EZI2C_FSM_WRITE_MASK & SCB_fsmState)))
                {
                ...
                SCB_curStatus |= (uint8) (locStatus & locIntrSlave);
            ...
            #if(SCB_EZI2C_EC_AM_ENABLE)
            {
                ...
                if(0u != (locIntrCause & SCB_INTR_CAUSE_I2C_EC))
                {
                    SCB_I2C_SLAVE_GENERATE_NACK; /* NACK in active mode */
                    SCB_ClearI2CExtClkInterruptSource(SCB_INTR_I2C_EC_WAKE_UP);
                }
            }
            #endif
    ...
    #ifdef SCB_EZI2C_NO_STRETCH_ISR_EXIT_CALLBACK
        SCB_EZI2C_NO_STRETCH_ISR_ExitCallback();
    #endif /* SCB_EZI2C_NO_STRETCH_ISR_EXIT_CALLBACK */

    }
#endif /* (SCB_EZI2C_SCL_STRETCH_DISABLE_CONST) */

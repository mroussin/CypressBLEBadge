...
static void SPI_Master_ScbModeStop(void)
{
    ...
    *******************************************************************************/
    void SPI_Master_SetPins(uint32 mode, uint32 subMode, uint32 uartEnableMask)
    {
        ...
        {
            SPI_Master_RX_SDA_MOSI_HSIOM_SEL_GPIO,
            SPI_Master_TX_SCL_MISO_HSIOM_SEL_GPIO,
            0u,
            0u,
            0u,
            0u,
            ...
            (SPI_Master_SCB_MODE_EZI2C == mode))
        {
            ...
            hsiomSel[SPI_Master_RX_SDA_MOSI_PIN_INDEX] = SPI_Master_RX_SDA_MOSI_HSIOM_SEL_I2C;
            pinsDm  [SPI_Master_RX_SDA_MOSI_PIN_INDEX] = SPI_Master_PIN_DM_OD_LO;
        #elif (SPI_Master_RX_WAKE_SDA_MOSI_PIN)
            hsiomSel[SPI_Master_RX_WAKE_SDA_MOSI_PIN_INDEX] = SPI_Master_RX_WAKE_SDA_MOSI_HSIOM_SEL_I2C;
            pinsDm  [SPI_Master_RX_WAKE_SDA_MOSI_PIN_INDEX] = SPI_Master_PIN_DM_OD_LO;
        #else
        #endif /* (SPI_Master_RX_SDA_MOSI_PIN) */
        
        #if (SPI_Master_TX_SCL_MISO_PIN)
            hsiomSel[SPI_Master_TX_SCL_MISO_PIN_INDEX] = SPI_Master_TX_SCL_MISO_HSIOM_SEL_I2C;
            pinsDm  [SPI_Master_TX_SCL_MISO_PIN_INDEX] = SPI_Master_PIN_DM_OD_LO;
        ...
        {
        #if (SPI_Master_RX_SDA_MOSI_PIN)
            hsiomSel[SPI_Master_RX_SDA_MOSI_PIN_INDEX] = SPI_Master_RX_SDA_MOSI_HSIOM_SEL_SPI;
        #elif (SPI_Master_RX_WAKE_SDA_MOSI_PIN)
            hsiomSel[SPI_Master_RX_WAKE_SDA_MOSI_PIN_INDEX] = SPI_Master_RX_WAKE_SDA_MOSI_HSIOM_SEL_SPI;
        #else
        #endif /* (SPI_Master_RX_SDA_MOSI_PIN) */
        
        #if (SPI_Master_TX_SCL_MISO_PIN)
            hsiomSel[SPI_Master_TX_SCL_MISO_PIN_INDEX] = SPI_Master_TX_SCL_MISO_HSIOM_SEL_SPI;
        #endif /* (SPI_Master_TX_SCL_MISO_PIN) */
            ...
            {
                /* Slave */
                pinsDm[SPI_Master_RX_SDA_MOSI_PIN_INDEX] = SPI_Master_PIN_DM_DIG_HIZ;
                pinsDm[SPI_Master_TX_SCL_MISO_PIN_INDEX] = SPI_Master_PIN_DM_STRONG;
                ...
                /* Disable input buffer */
                 pinsInBuf |= SPI_Master_TX_SCL_MISO_PIN_MASK;
            #endif /* (SPI_Master_TX_SCL_MISO_PIN) */
            }
            else 
            {
                /* (Master) */
                pinsDm[SPI_Master_RX_SDA_MOSI_PIN_INDEX] = SPI_Master_PIN_DM_STRONG;
                pinsDm[SPI_Master_TX_SCL_MISO_PIN_INDEX] = SPI_Master_PIN_DM_DIG_HIZ;
            ...
            #if (SPI_Master_RX_SDA_MOSI_PIN)
                pinsInBuf |= SPI_Master_RX_SDA_MOSI_PIN_MASK;
            #elif (SPI_Master_RX_WAKE_SDA_MOSI_PIN)
                pinsInBuf |= SPI_Master_RX_WAKE_SDA_MOSI_PIN_MASK;
            #else
            #endif /* (SPI_Master_RX_SDA_MOSI_PIN) */
        ...
        else /* UART */
        {
            if (SPI_Master_UART_MODE_SMARTCARD == subMode)
            {
                /* SmartCard */
            #if (SPI_Master_TX_SCL_MISO_PIN)
                hsiomSel[SPI_Master_TX_SCL_MISO_PIN_INDEX] = SPI_Master_TX_SCL_MISO_HSIOM_SEL_UART;
                pinsDm  [SPI_Master_TX_SCL_MISO_PIN_INDEX] = SPI_Master_PIN_DM_OD_LO;
            #endif /* (SPI_Master_TX_SCL_MISO_PIN) */
            }
            else /* Standard or IrDA */
            {
                ...
                {
                #if (SPI_Master_RX_SDA_MOSI_PIN)
                    hsiomSel[SPI_Master_RX_SDA_MOSI_PIN_INDEX] = SPI_Master_RX_SDA_MOSI_HSIOM_SEL_UART;
                    pinsDm  [SPI_Master_RX_SDA_MOSI_PIN_INDEX] = SPI_Master_PIN_DM_DIG_HIZ;
                #elif (SPI_Master_RX_WAKE_SDA_MOSI_PIN)
                    hsiomSel[SPI_Master_RX_WAKE_SDA_MOSI_PIN_INDEX] = SPI_Master_RX_WAKE_SDA_MOSI_HSIOM_SEL_UART;
                    pinsDm  [SPI_Master_RX_WAKE_SDA_MOSI_PIN_INDEX] = SPI_Master_PIN_DM_DIG_HIZ;
                #else
                #endif /* (SPI_Master_RX_SDA_MOSI_PIN) */
                }
    ...
    /* Note: the DR register settings do not effect the pin output if HSIOM is other than GPIO */
        ...
        SPI_Master_SET_HSIOM_SEL(SPI_Master_RX_SDA_MOSI_HSIOM_REG,
                                       ...
                                       SPI_Master_RX_SDA_MOSI_HSIOM_POS,
                                        hsiomSel[SPI_Master_RX_SDA_MOSI_PIN_INDEX]);

        SPI_Master_uart_rx_i2c_sda_spi_mosi_SetDriveMode((uint8) pinsDm[SPI_Master_RX_SDA_MOSI_PIN_INDEX]);

        #if (!SPI_Master_CY_SCBIP_V1)
            SPI_Master_SET_INP_DIS(SPI_Master_uart_rx_i2c_sda_spi_mosi_INP_DIS,
                                         SPI_Master_uart_rx_i2c_sda_spi_mosi_MASK,
                                         (0u != (pinsInBuf & SPI_Master_RX_SDA_MOSI_PIN_MASK)));
        #endif /* (!SPI_Master_CY_SCBIP_V1) */
        ...
        SPI_Master_SET_HSIOM_SEL(SPI_Master_RX_WAKE_SDA_MOSI_HSIOM_REG,
                                       ...
                                       hsiomSel[SPI_Master_RX_WAKE_SDA_MOSI_PIN_INDEX]);

        SPI_Master_uart_rx_wake_i2c_sda_spi_mosi_SetDriveMode((uint8)
                                                               pinsDm[SPI_Master_RX_WAKE_SDA_MOSI_PIN_INDEX]);

        SPI_Master_SET_INP_DIS(SPI_Master_uart_rx_wake_i2c_sda_spi_mosi_INP_DIS,
                                     SPI_Master_uart_rx_wake_i2c_sda_spi_mosi_MASK,
                                     (0u != (pinsInBuf & SPI_Master_RX_WAKE_SDA_MOSI_PIN_MASK)));

         /* Set interrupt on falling edge */
        SPI_Master_SET_INCFG_TYPE(SPI_Master_RX_WAKE_SDA_MOSI_INTCFG_REG,
                                        SPI_Master_RX_WAKE_SDA_MOSI_INTCFG_TYPE_MASK,
    ...
    #if (SPI_Master_TX_SCL_MISO_PIN)
        SPI_Master_SET_HSIOM_SEL(SPI_Master_TX_SCL_MISO_HSIOM_REG,
                                       SPI_Master_TX_SCL_MISO_HSIOM_MASK,
                                       SPI_Master_TX_SCL_MISO_HSIOM_POS,
                                        hsiomSel[SPI_Master_TX_SCL_MISO_PIN_INDEX]);

        SPI_Master_uart_tx_i2c_scl_spi_miso_SetDriveMode((uint8) pinsDm[SPI_Master_TX_SCL_MISO_PIN_INDEX]);

    #if (!SPI_Master_CY_SCBIP_V1)
        SPI_Master_SET_INP_DIS(SPI_Master_uart_tx_i2c_scl_spi_miso_INP_DIS,
                                     SPI_Master_uart_tx_i2c_scl_spi_miso_MASK,
                                    (0u != (pinsInBuf & SPI_Master_TX_SCL_MISO_PIN_MASK)));
    #endif /* (!SPI_Master_CY_SCBIP_V1) */
    #endif /* (SPI_Master_RX_SDA_MOSI_PIN) */
